...
service httpd stop
cp -r /vagrant/sources/httpd.conf /etc/httpd/conf/
cp -r /vagrant/sources/workers.properties /etc/httpd/conf.d/
cp -r /vagrant/sources/vhost.conf /etc/httpd/conf.d/
...
cp -r /vagrant/sources/tomcat /etc/init.d/

#Set -i permisson for iptables and chagne this file to correct 
cd /etc/sysconfig
chattr -i iptables
cp -r /vagrant/sources/iptables /etc/sysconfig/
